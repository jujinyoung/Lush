...
public class OrderTradePlusAjax implements CommandHandler {
	...
	public String process(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String[] arrayParam = request.getParameterValues("pdidlist[]");
		for (int i = 0; i < arrayParam.length; i++) {
			if (i % 2 == 0) { // psid
				pdidlist.add(Long.parseLong(arrayParam[i]));
			} else { // customer amount
				customerAmountList.add(Long.parseLong(arrayParam[i]));
			}
		}

		orderService.plusTrade(pdidlist, customerAmountList);
