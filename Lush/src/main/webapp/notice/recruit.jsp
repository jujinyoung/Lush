		...
		<article class="my-contents notice">
			...
			<div class="tab-table-wrap">
				<ul class="tab-btn left" id="tabList">
					<li id="default" ><a href="/Lush/notice/list.do" data-tab="1" class="tabmenu">공지사항</a></li>
					<li><a href="/Lush/notice/list2.do" data-tab="2" class="tabmenu on" >직원채용</a></li>
				</ul>

				<div class="tab-cont">
					<!-- 공지사항 페이지 -->
						<div class="tab-inner notice" id="noticeList_1">
					...
					<!-- 직원채용 페이지 -->
					<div class="tab-inner recruit on" id="noticeList_2">
						<table class="list-table notice-table">
								...
								<thead>
								 ...
								 <tbody>
							<c:choose>
								<c:when test="${ not empty recruits }">
									<c:forEach items="${ recruits }" var="dto">

										<tr>
											<td>${dto.rec_id}</td>
											<td><a
												href="/Lush/notice/view2.do?rec_id=${dto.rec_id}">${dto.rec_title}</a></td>
											<td>${dto.rec_date}</td>
										</tr>
									</c:forEach>
								</c:when>
